...
pub fn drawTitleMenu(menu: *MenuState, ui_style: ui_mod.UiStyle, status_message: *StatusMessage, screen_width: f32, screen_height: f32) TitleMenuAction {
    ...
    const title_w = engine.Text.measure(title, title_size);
    const title_x: i32 = @intFromFloat((screen_width - @as(f32, @floatFromInt(title_w))) / 2.0);
    engine.Text.draw(title, title_x, @intFromFloat(screen_height * 0.12), title_size, ui_style.text);

    const sub_w = engine.Text.measure(subtitle, subtitle_size);
    const sub_x: i32 = @intFromFloat((screen_width - @as(f32, @floatFromInt(sub_w))) / 2.0);
    engine.Text.draw(subtitle, sub_x, @as(i32, @intFromFloat(screen_height * 0.12)) + title_size + 8, subtitle_size, ui_style.text_muted);
...
pub fn drawWorldListMenu(menu: *MenuState, ui_style: ui_mod.UiStyle, status_message: *StatusMessage, screen_width: f32, screen_height: f32) WorldMenuAction {
    ...
    const header_w = engine.Text.measure(header, ui_style.font_size);
    engine.Text.draw(header, @intFromFloat((screen_width - @as(f32, @floatFromInt(header_w))) / 2.0), @intFromFloat(24.0 * ui_style.scale), ui_style.font_size, ui_style.text);
    ...
    if (menu.worlds.len == 0) {
        engine.Text.draw("No worlds found.", @intFromFloat(list_x), @intFromFloat(list_y), ui_style.small_font_size, ui_style.text_muted);
        engine.Text.draw("Create a new world to begin.", @intFromFloat(list_x), @intFromFloat(list_y + 22.0 * ui_style.scale), ui_style.small_font_size, ui_style.text_muted);
    }
...
fn updateNameInput(input: *NameInput) void {
    var c: i32 = engine.Input.Keyboard.getCharPressed();
    while (c != 0) : (c = engine.Input.Keyboard.getCharPressed()) {
        if (c >= 32 and c <= 126) {
            input.pushAscii(@intCast(c));
        }
    }

    if (engine.Input.Keyboard.isPressed(engine.KeyboardKey.backspace)) {
        input.pop();
    }
}
...
pub fn drawCreateWorldMenu(menu: *MenuState, ui_style: ui_mod.UiStyle, status_message: *StatusMessage, screen_width: f32, screen_height: f32) CreateWorldResult {
    ...
    engine.Text.draw(display, @intFromFloat(field_x + 14.0 * ui_style.scale), @intFromFloat(field_y + 16.0 * ui_style.scale), ui_style.font_size, if (name.len == 0) ui_style.text_muted else ui_style.text);

    if (engine.Input.Keyboard.isPressed(engine.KeyboardKey.enter)) {
    ...
    engine.Text.draw("Press Enter to create.", @intFromFloat(field_x), @intFromFloat(field_y + field_h + 10.0 * ui_style.scale), ui_style.small_font_size, ui_style.text_muted);
    return .none;
}
...
pub fn drawPauseMenu(menu: *MenuState, ui_style: ui_mod.UiStyle, status_message: *StatusMessage, screen_width: f32, screen_height: f32) PauseMenuAction {
    _ = status_message;
    engine.Shapes.drawRectangleRec(engine.Rectangle{ .x = 0, .y = 0, .width = screen_width, .height = screen_height }, engine.Color{ .r = 0, .g = 0, .b = 0, .a = 120 });

    const header = "Paused";
    const header_w = engine.Text.measure(header, ui_style.font_size);
    engine.Text.draw(header, @intFromFloat((screen_width - @as(f32, @floatFromInt(header_w))) / 2.0), @intFromFloat(screen_height * 0.2), ui_style.font_size, ui_style.text);
